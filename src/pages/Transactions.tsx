...
import { CreditCard, ArrowDownRight, ArrowUpRight, Trash2, Edit2, Image as ImageIcon, Repeat, ListOrdered, X, Smile } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <Button onClick={handleOpenNew} className="shadow-lg shadow-sky-500/20">Novo Lançamento</Button>
      </header>
              ...
              <div key={tx.id} className="p-4 sm:px-6 flex items-center justify-between hover:bg-[#1E293B] transition-colors group">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-[#1E293B] flex items-center justify-center text-xl shadow-inner border border-slate-800/60 overflow-hidden shrink-0 relative">
                    {tx.imageUrl ? (
                      <img src={tx.imageUrl} alt={tx.description} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {tx.isFixed && <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded-md bg-amber-500/10 text-amber-500 border border-amber-500/20">Fixa</span>}
                      {tx.installmentInfo && <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded-md bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">Parc. {tx.installmentInfo}</span>}
                    </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? "Editar Lançamento" : "Novo Lançamento"}>
        <form onSubmit={handleAddTransaction} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input 
              label={installments > 1 && !editingId ? "Valor Total (R$)" : "Valor (R$)"} 
              type="number" 
              step="0.01"
              placeholder="0.00"
              value={amount}
          ...
          <div className="flex gap-4 relative">
            <div className="space-y-2 w-24 shrink-0">
              <label className="text-sm font-medium leading-none text-slate-300">Ícone</label>
              <button
                type="button"
            ...
            <div className="flex-1 space-y-2">
              <label className="text-sm font-medium leading-none text-slate-300">Foto / Comprovante</label>
              <div className="flex items-center gap-3">
                ...
                {imageBase64 && (
                  <button type="button" onClick={() => setImageBase64(null)} className="p-2 text-slate-400 hover:text-rose-400">
                    <X className="w-5 h-5" />
                  </button>
                )}
              </div>
          ...
          {!editingId && (
            <div className="p-4 bg-[#0B1121] rounded-xl border border-slate-800/60 space-y-4">
              <h4 className="text-sm font-medium text-slate-300 mb-2">Recorrência e Parcelamento</h4>
              
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Repeat className="w-4 h-4 text-amber-500" />
                  <span className="text-sm text-slate-300">É uma despesa fixa mensal?</span>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" checked={isFixed} onChange={(e) => { setIsFixed(e.target.checked); if (e.target.checked) setInstallments(1); }} className="sr-only peer" />
                ...
                <div className="flex flex-col gap-2 pt-2 border-t border-slate-800">
                  ...
                  {installments > 1 && (
                    <p className="text-xs text-slate-400 mt-1">
                      O valor total de R$ {amount || '0'} será dividido em {installments} vezes de R$ {(Number(amount) / installments).toFixed(2)}.
                    </p>
                  )}
                </div>
              )}
