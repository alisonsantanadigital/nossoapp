...
export const usernameToEmail = (username: string) => `alisonsantanadigital+${username.trim().toLowerCase()}@gmail.com`;
...
export function Login() {
  ...
  const handleResetPassword = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setSuccess('Instruções de redefinição foram enviadas para o email do administrador (alisonsantanadigital@gmail.com).');
      setError('');
    } catch (err) {
      setError('Erro ao solicitar redefinição. Verifique se o usuário está correto.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAutoCreateAccounts = async () => {
    ...
    try {
      ...
      try {
        const { user: user1 } = await createUserWithEmailAndPassword(auth, emailAlison, 'alisonsan123');
      ...
      try {
        const { user: user2 } = await createUserWithEmailAndPassword(auth, emailLaysa, 'laysan123');
      ...
      setSuccess('Contas "alisonsan" e "laysan" criadas com sucesso! Você pode fazer o login acima.');
  ...
  return (
    <div className="min-h-screen bg-[#0B1121] flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-md bg-[#131B2F] p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#1E293B] mb-6">
        ...
        <h1 className="text-2xl font-bold text-center text-white mb-2">Bem-vindo à Nossa Casa</h1>
        <p className="text-center text-slate-400 mb-8 text-sm">O painel de controle financeiro da sua família.</p>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {success && <p className="text-sky-400 text-sm text-center font-medium bg-sky-400/10 p-2 rounded-lg">{success}</p>}
          
          <div className="flex items-center justify-between mt-2 mb-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" className="rounded text-sky-400 focus:ring-teal-600 w-4 h-4 border-slate-700" />
              <span className="text-sm text-slate-300">Lembrar-me</span>
            </label>
            <button onClick={handleResetPassword} type="button" className="text-sm font-medium text-sky-400 hover:text-sky-300">Esqueci a senha</button>
          </div>
        ...
        <p className="text-center text-slate-400 text-sm mt-8">
          Ainda não tem uma conta? <Link to="/register" className="font-medium text-sky-400 hover:text-sky-300">Criar agora</Link>
        </p>
      </div>
